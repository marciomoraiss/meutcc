...
import { orientationSlots, formatSlot, weekStartFor, SCHEDULE_TIME_ZONE, WEEKLY_LIMIT } from "../lib/schedule";
...
type MvpData = {
  ...
  deliveries?: Array<{ id: number; kind: string; version: number; status: string; dueAt: string | null; fileKey: string | null; fileName: string | null; advisorNote?: string | null }>;
...
export default function Home() {
  ...
  const syncMvp = useCallback((data: MvpData) => {
    ...
    setBookedCount((data.busySlots ?? []).filter(value => weekStartFor(value) === weekStartFor(new Date())).length);
    ...
    if (data.messages) setMessages(data.messages.map((item) => ({ author: typeof item.authorName === "string" && item.authorName.trim() ? item.authorName : item.authorRole === "Orientador" ? "Orientador" : "Aluno", role: item.authorRole, text: item.body, time: new Date(item.createdAt).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }), read: item.readAt ? "lida" : "enviada" })));
  }, []);
  ...
  useEffect(() => {
    if (page !== "schedule" || !accessToken) return;
    const refresh = () => { void loadMvp(accessToken).catch(() => {}); };
    refresh();
    const timer = window.setInterval(refresh, 30000);
    return () => window.clearInterval(timer);
  }, [page, accessToken, loadMvp]);
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <div className="content">
          {role === "advisor" && page === "dashboard" && <AdvisorDashboard onOpenStudent={() => setPage("student")} onSchedule={() => setPage("schedule")} onOpenClassroom={() => setPage("classroom")} bookedCount={bookedCount} liveStudents={mvp?.students ?? []} appointments={(mvp?.appointments ?? []).filter(item => weekStartFor(item.startsAt) === weekStartFor(new Date()))} invitations={mvp?.invitations ?? []} />}
          ...
          {role === "advisor" && page === "student" && (mvp?.focusTcc ? <StudentRecord reviewStatus={reviewStatus} setReviewStatus={async (value, note, deliveryId) => { try { await postAction({ action: "review", status: value, note, deliveryId }); notify(value === "Aprovado" ? "Entrega aprovada e progresso atualizado." : "Parecer e ajustes registrados para o aluno."); } catch (error) { notify(error instanceof Error ? error.message : "Erro ao avaliar"); } }} onMentoring={() => setPage("mentoring")} deliveries={mvp?.deliveries} student={mvp.students?.[0]} /> : <EmptyState title="Nenhum aluno vinculado" text="Importe a lista da sua orientação quando a distribuição de orientadores estiver concluída." action="Abrir cadastro da turma" onAction={() => setPage("classroom")} />)}
          {role === "student" && page === "dashboard" && <StudentDashboard name={profileName} advisorNote={mvp?.deliveries?.at(-1)?.advisorNote} progress={mvp?.focusTcc?.progress ?? 0} deliveryStatus={deliveryStatus} submitDelivery={async (file, note, kind) => { try { const form = new FormData(); form.append("file", file); const uploadedResponse = await fetch("/api/files", { method: "POST", headers: { authorization: `Bearer ${accessToken}` }, body: form }); const uploaded = await uploadedResponse.json() as { error?: string; key?: string; name?: string }; if (!uploadedResponse.ok || !uploaded.key) throw new Error(uploaded.error ?? "Erro no envio"); await postAction({ action: "submit", kind, fileKey: uploaded.key, fileName: uploaded.name, note }); notify("Entrega enviada. O orientador foi notificado."); } catch (error) { notify(error instanceof Error ? error.message : "Erro no envio"); } }} onSchedule={() => setPage("schedule")} onMentoring={() => setPage("mentoring")} />}
          {role === "student" && page === "student" && <MyTcc deliveryStatus={deliveryStatus} tcc={mvp?.focusTcc} />}
          {page === "schedule" && <Schedule role={role} appointments={mvp?.appointments ?? []} busySlots={mvp?.busySlots ?? []} studentId={mvp?.profile?.id ?? ""} weeklyLimit={mvp?.cohort?.weeklyLimit ?? WEEKLY_LIMIT} onBook={async (startsAt) => { try { await postAction({ action: "book", startsAt, subject: "Orientação de TCC" }); notify("Agendamento confirmado e registrado no histórico."); } catch (error) { notify(error instanceof Error ? error.message : "Erro ao agendar"); if (accessToken) await loadMvp(accessToken).catch(() => {}); } }} onCancel={async (id) => { try { await postAction({ action: "cancel", id }); notify("Agendamento cancelado. A vaga foi liberada."); } catch (error) { notify(error instanceof Error ? error.message : "Erro ao cancelar"); } }} />}
...
function StudentDashboard({ name, advisorNote, progress, deliveryStatus, submitDelivery, onSchedule, onMentoring }: { name: string; advisorNote?: string | null; progress: number; deliveryStatus: string; submitDelivery: (file: File, note: string, kind: string) => Promise<void>; onSchedule: () => void; onMentoring: () => void }) {
...
function Schedule({ role, appointments, busySlots, studentId, weeklyLimit, onBook, onCancel }: {
  role: Role; appointments: NonNullable<MvpData["appointments"]>; busySlots: string[]; studentId: string;
  weeklyLimit: number; onBook: (slot: string) => Promise<void>; onCancel: (id: number) => Promise<void>;
}) {
  const slots = orientationSlots();
  const weeks = [...new Set(slots.map(slot => slot.weekStart))];
  const [now, setNow] = useState(Date.now);
  const [week, setWeek] = useState(() => weeks.find(value => slots.some(slot => slot.weekStart === value && new Date(slot.startsAt).getTime() > Date.now())) ?? weeks.at(-1) ?? "");
  const [pending, setPending] = useState(false);
  useEffect(() => { const timer = window.setInterval(() => setNow(Date.now()), 30000); return () => window.clearInterval(timer); }, []);
  const shown = slots.filter(slot => slot.weekStart === week);
  const busy = new Set(busySlots.map(value => new Date(value).getTime()));
  const count = busySlots.filter(value => weekStartFor(value) === week).length;
  const own = appointments.find(item => item.studentId === studentId && weekStartFor(item.startsAt) === week);
  const available = shown.filter(slot => new Date(slot.startsAt).getTime() > now && !busy.has(new Date(slot.startsAt).getTime()));
  const act = async (task: () => Promise<void>) => { if (pending) return; setPending(true); try { await task(); } finally { setPending(false); } };
  return <>
    <PageHead eyebrow={role === "advisor" ? "Gestão de horários" : "Orientação presencial"} title={role === "advisor" ? "Agenda de orientações" : "Agendar orientação"} text="Setembro a novembro de 2026 · horário de Brasília · 15 minutos por atendimento" />
    <section className="panel"><label><span>Semana de atendimento</span><select value={week} onChange={event => setWeek(event.target.value)} disabled={pending}>{weeks.map(value => <option key={value} value={value}>Semana de {new Date(value + "T12:00:00Z").toLocaleDateString("pt-BR", { timeZone: SCHEDULE_TIME_ZONE })}</option>)}</select></label><p>Terças, 18h–19h · Sextas, 11h–12h. Datas passadas não aceitam reservas.</p></section>
    <div className="stats"><Stat label="Atendimentos da semana" value={`${count}/${weeklyLimit}`} note="Até oito encontros presenciais" /><Stat label="Vagas futuras disponíveis" value={String(available.length)} note="Horários passados e ocupados excluídos" tone="red" /><Stat label={role === "advisor" ? "Duração" : "Meu encontro nesta semana"} value={role === "advisor" ? "15 minutos" : own ? "Confirmado" : "Nenhum"} note={own ? formatSlot(own.startsAt) : "Um encontro por aluno por semana"} tone="gold" /></div>
    <div className="schedule-grid"><section className="panel"><div className="panel-head"><div><h2>Horários da semana</h2><p>Presencial · duração de 15 minutos · Brasília</p></div><span className={`capacity ${count >= weeklyLimit ? "full" : ""}`}>{count >= weeklyLimit ? "Agenda completa" : available.length ? "Vagas abertas" : "Sem vagas futuras"}</span></div>
      <div className="slot-grid">{shown.map(slot => {
        const stamp = new Date(slot.startsAt).getTime();
        const isOwn = !!own && new Date(own.startsAt).getTime() === stamp;
        const occupied = busy.has(stamp);
        const past = stamp <= now;
        const appointment = role === "advisor" ? appointments.find(item => new Date(item.startsAt).getTime() === stamp) : undefined;
        return <button key={slot.startsAt} className={isOwn ? "slot selected" : "slot"} disabled={role === "advisor" || pending || past || occupied || !!own || count >= weeklyLimit} onClick={() => void act(() => onBook(slot.startsAt))}><strong>{slot.label}</strong><small>{isOwn ? "Seu encontro" : past ? "Encerrado" : occupied ? "Reservado" : "Disponível"}</small>{appointment && <small>{appointment.studentEmail}</small>}</button>;
      })}</div>
      {role === "student" && own && <div className="booking-confirm"><span><strong>Encontro confirmado</strong><small>{formatSlot(own.startsAt)} · 15 minutos · local a confirmar</small></span>{new Date(own.startsAt).getTime() > now && <button className="secondary danger" disabled={pending} onClick={() => void act(() => onCancel(own.id))}>Cancelar</button>}</div>}
    </section><aside className="panel rules"><h2>Regras automáticas</h2><Rule title="Teto do orientador" value="Até 8 presenciais por semana" /><Rule title="Limite do aluno" value="1 encontro presencial por semana" /><Rule title="Dias e horários" value="Terças 18h–19h · Sextas 11h–12h" /><Rule title="Período" value="Setembro, outubro e novembro de 2026" /><Rule title="Conflito de horário" value="Horário reservado não aceita outra reserva" /><Rule title="Histórico" value="Cancelamentos são preservados no banco" /></aside></div>
  </>;
}
